...
class CHSShoutbox implements CHSModule
{
 ...
 private function printSingleShout($value)
 {
  ...
  $value = explode("\t", $value);
  $value[3] = preg_replace_callback("/([^ ^>]+?:\/\/|www\.)[^ ^<^\.]+(\.[^ ^<^\.]+)+/si", create_function('$arr', "return \$arr[2] ? '<a href=\"' . (\$arr[1] == 'www.' ? 'http://' : '') . \$arr[0] . '\" target=\"blank\">' . (strlen(\$arr[0]) > 25 ? substr(\$arr[0], 0, 15) . '...' . substr(\$arr[0], -10) : \$arr[0]) . '</a>' : \$arr[0];"), $value[3]);
